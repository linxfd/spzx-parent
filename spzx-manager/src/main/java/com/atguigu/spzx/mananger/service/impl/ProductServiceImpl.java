...
import com.atguigu.spzx.mananger.mapper.ProductDetailsMapper;
import com.atguigu.spzx.mananger.mapper.ProductMapper;
import com.atguigu.spzx.mananger.mapper.ProductSkuMapper;
...
public class ProductServiceImpl implements ProductService {
    ...
    public void save(Product product) {
        ...
        for(int i=0,size=productSkuList.size(); i<size; i++) {
            ...
            productSku.setSkuCode(product.getId() + "_" + i);       // 构建skuCode商品编号
            productSku.setProductId(product.getId());               // 设置商品id
            productSku.setSkuName(product.getName() + productSku.getSkuSpec());  // 构建skuname名称
            productSku.setSaleNum(0);                               // 设置销量
            productSku.setStatus(0);                                // 设置上架状态为0
        ...
        productDetails.setImageUrls(product.getDetailsImageUrls());  // 设置详情图列表
        productDetailsMapper.save(productDetails);   // 保存数据
    }
